...
extern "C" {
	...
	***************************************/
	...
	参数：nChannel[in] 通道号
	***************************************/
	LPKERNELEX_API BOOL WINAPI LPR_InitEx(int  nChannel=1);

	/**************************************
	LPR_UninitEx卸载核心库
	参数：nChannel[in] 通道号
	***************************************/
	LPKERNELEX_API void WINAPI LPR_UninitEx(int  nChannel=1);
	...
	nChannel[in]   通道号
	***************************************/
	LPKERNELEX_API BOOL WINAPI LPR_RGB888Ex(unsigned char *pImg, int nWidth, int  nHeight, TH_PlateResult* pResult, int &nRecogNum, TH_RECT *prcRange, int  nChannel=1);
	...
	用此函数获取识别到车牌的帧内存。
	***************************************/
	LPKERNELEX_API BOOL WINAPI LPR_GetImageBuf(unsigned char *&pImageBuf, int &nWidth, int &nHeight, int &nSize, int  nChannel=1);
	...
	nChannel[in]         通道号
	***************************************/
	LPKERNELEX_API BOOL WINAPI LPR_FileEx(char* lpszFileName, char *lpszPlateFile, TH_PlateResult* pResult, int &nRecogNum, TH_RECT *prcRange,int nChannel=1);
