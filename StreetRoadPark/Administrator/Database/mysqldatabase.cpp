    ...
    QObject(parent)
{
    ...
    pCodec = GetTextCodec( );
    bReconnect = mysql_thread_safe();//0415
    hStmt = NULL;
}
...
QTextCodec *CMySqlDatabase::GetTextCodec()
{
    ...
    pCodec = QTextCodec::codecForName( strText.toUtf8() );
    return pCodec;
}
...
bool CMySqlDatabase::DbCrud( QString& strSql, QString& strError )
{
    ...
    //QByteArray byteSql = GetTextCodec()->fromUnicode( strSql );
    QByteArray byteSql = strSql.toUtf8();// 20130418
...
bool CMySqlDatabase::BlobReadDb( QByteArray &byData, QString &strSql, QString &strError )
{
    ...
    byteSql = GetTextCodec()->fromUnicode( strSql );
    nSize = byteSql.count( );
    pSql = byteSql.data( );
