...
{      
	size_t nBlockSize = this->MandatoryBlockSize();
	int nRes = ( length % nBlockSize );
 
	if ( nRes != 0 ) {
		char cInfo[ 256 ] = { 0 };
		sprintf( cInfo, "length = %d MandatoryBlockSize( ) = %d", length, nBlockSize );
		//assert(length % nBlockSize == 0);

		#ifdef _DEBUG
		_CrtDbgReport( _CRT_ASSERT, NULL, NULL, cInfo, NULL );
		#endif
	}
